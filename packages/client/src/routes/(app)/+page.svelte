<script>
    import { goto } from "$app/navigation"
    import { inViewPopIn } from "$lib/motion.svelte"
    import {
        ArrowDown,
        CheckSquare,
        ChevronDown,
        Download,
        FolderCode,
        ImageUpscale,
        Users,
    } from "@lucide/svelte"
    import Button from "../../components/Button.svelte"
    import DownloadSection from "../../components/DownloadSection.svelte"
    import PixelsTransition from "../../components/PixelsTransition.svelte"
    import { m } from "../../paraglide/messages"
</script>

<section
    class="relative flex flex-col justify-center items-center text-center gap-8 min-h-screen
    mt-[-60px] overflow-hidden
    bg-gradient-to-b from-[#000000] via-[#0f2d43] to-[#4180c8]"
>
    <div
        class="absolute pointer-events-none left-1/2 -translate-x-1/2 bottom-[60vh] size-[500px] md:size-[1000px] opacity-60"
    >
        <div class="relative">
            <div
                style="animation: pulse 20s cubic-bezier(0.4, 0, 0.6, 1) infinite"
                class="absolute inset-0 rounded-full blur-[3000px] bg-[#9ed3ff] opacity-50"
            ></div>
            <img
                src="bm-highres.webp"
                alt=""
                draggable="false"
                class=" spin drop-shadow-[0_0_60px_rgba(0,120,255,0.3)]"
                style="animation: spin 300s linear infinite;"
            />
        </div>
    </div>

    <h1
        use:inViewPopIn={{
            delay: 0.3,
            duration: 0.6,
        }}
        class="text-4xl md:text-6xl font-bold text-white drop-shadow-[0_0_20px_rgba(0,0,0,0.6)]"
    >
        {m.home_section1_title()}
    </h1>
    <h2
        use:inViewPopIn={{
            delay: 1,
            duration: 0.6,
        }}
        class="text-lg md:text-xl xl:text-3xl opacity-80 max-w-2xl text-balance"
    >
        {m.home_section1_subtitle()}
    </h2>
    <div
        use:inViewPopIn={{
            delay: 2,
            duration: 0.6,
        }}
    >
        <Button
            iconAnimations
            onclick={() => goto("#install")}
            className="btn-neutral btn-lg min-w-80"
            >{m.home_section1_cta()}
            {#snippet itemBefore()}
                <Download class="w-4 h-4" />
            {/snippet}
        </Button>
    </div>
    <a
        href="#goal"
        use:inViewPopIn={{
            delay: 2.3,
            duration: 0.6,
        }}
        class="flex flex-col items-center gap-2 self-center mt-8"
        >{m.home_section1_learnmore()}
        <ChevronDown class="w-4 h-4" />
    </a>
    <div class="absolute inset-0 pointer-events-none">
        <div
            class="w-full h-full bg-[radial-gradient(circle,_rgba(255,255,255,0.4)_1px,_transparent_1px)] [background-size:16px_16px] opacity-20"
        ></div>
    </div>
</section>
<PixelsTransition from="#4180c8" to="var(--color-base-100)" />
<section
    id="goal"
    class="flex flex-col md:px-20 lg:px-40 px-4 py-10 items-center"
>
    <code class="text-primary scale-y-[.85]">{m.home_goal_subtitle()}</code>
    <h1 class="text-4xl font-semibold">{m.home_goal_title()}</h1>

    <p class="text-xl mt-8">
        {@html m.home_goal_description()}
    </p>

    <code class="flex items-center gap-2 text-primary scale-y-[.85] mt-8">
        <ArrowDown class="size-4" />
        {m.home_goal_showcase()}
        <ArrowDown class="size-4" /></code
    >
    <video
        src="showcase.mp4"
        class="rounded-box mt-8 border-2 border-primary/40 ring-8 ring-primary/10 max-w-full"
        autoplay
        muted
        loop
    />
</section>
<section id="features" class="flex flex-col md:px-20 lg:px-40 px-4 py-10">
    <code class="text-primary scale-y-[.85]">{m.home_section2_subtitle()}</code>
    <h1 class="text-4xl font-semibold">{m.home_section2_title()}</h1>
    <ul class="flex flex-col md:grid md:grid-cols-2 gap-8 md:gap-4 mt-8">
        <li
            class="flex flex-col gap-4 p-4 rounded-box border border-base-200 overflow-hidden bg-[radial-gradient(circle,_rgba(255,255,255,0.05)_1px,_transparent_1px)] [background-size:20px_20px]"
        >
            <div class="relative">
                <div
                    class="absolute top-0 left-0 w-full h-[20px] bg-primary blur-2xl rounded-full"
                ></div>
            </div>
            <span class="flex gap-2 items-center">
                <ImageUpscale class="size-6" />
                <h2 class="text-2xl font-semibold">
                    {m.home_section2_item1_title()}
                </h2>
            </span>
            <p>
                {m.home_section2_item1_description()}
            </p>
        </li>
        <li
            class="flex flex-col gap-4 p-4 rounded-box border border-base-200 overflow-hidden bg-[radial-gradient(circle,_rgba(255,255,255,0.05)_1px,_transparent_1px)] [background-size:20px_20px]"
        >
            <div class="relative">
                <div
                    class="absolute top-0 left-0 w-full h-[20px] bg-primary blur-2xl rounded-full"
                ></div>
            </div>
            <span class="flex gap-2 items-center">
                <CheckSquare class="size-6" />
                <h2 class="text-2xl font-semibold">
                    {m.home_section2_item2_title()}
                </h2>
            </span>
            <p>
                {m.home_section2_item2_description()}
            </p>
        </li>
        <li
            class="flex flex-col gap-4 p-4 rounded-box border border-base-200 overflow-hidden bg-[radial-gradient(circle,_rgba(255,255,255,0.05)_1px,_transparent_1px)] [background-size:20px_20px]"
        >
            <div class="relative">
                <div
                    class="absolute top-0 left-0 w-full h-[20px] bg-primary blur-2xl rounded-full"
                ></div>
            </div>
            <span class="flex gap-2 items-center">
                <FolderCode class="size-6" />
                <h2 class="text-2xl font-semibold">
                    {m.home_section2_item3_title()}
                </h2>
            </span>
            <p>
                {@html m.home_section2_item3_description()}
            </p>
        </li>
        <li
            class="flex flex-col gap-4 p-4 rounded-box border border-base-200 overflow-hidden bg-[radial-gradient(circle,_rgba(255,255,255,0.05)_1px,_transparent_1px)] [background-size:20px_20px]"
        >
            <div class="relative">
                <div
                    class="absolute top-0 left-0 w-full h-[20px] bg-primary blur-2xl rounded-full"
                ></div>
            </div>
            <span class="flex gap-2 items-center">
                <Users class="size-6" />
                <h2 class="text-2xl font-semibold">
                    {m.home_section2_item4_title()}
                </h2>
            </span>
            <p>
                {@html m.home_section2_item4_description()}
            </p>
        </li>
    </ul>
</section>
<DownloadSection />
<PixelsTransition from="var(--color-base-100)" to="var(--color-primary)" />
<section
    id="usage"
    class="w-full py-10 bg-primary text-primary-content min-h-screen lg:px-40 md:px-20 px-4 flex flex-col gap-4"
>
    <code class="text-base-100 scale-y-[.85] self-center"
        >{m.home_section3_subtitle()}</code
    >
    <h1 class="text-4xl self-center">{m.home_section3_title()}</h1>
    <p class="mb-8 mt-4">
        {m.home_section3_description()}
    </p>
    <span class="flex gap-2 items-end">
        <h2 class="text-6xl">1.</h2>
        <p class="text-xl">
            {m.home_section3_step1()}
        </p>
    </span>
    <img
        src="guide/usage/1.png"
        class="mt-4 opacity-90 max-w-[40rem] w-full self-center rounded-box"
        alt="Screenshot showing 4 coordinates mapped from the wplace.live layout to Blue Marble GUI."
    />
    <div class="alert alert-warning items-start gap-2 flex flex-col">
        <b class="text-xl">{m.home_section3_image_title()}</b>
        <p>
            {m.home_section3_image_desc_1()}
            <a
                href="https://pepoafonso.github.io/color_converter_wplace/"
                class="href"
                target="_blank">{m.home_section3_image_desc_2()}</a
            >.
        </p>
        <p>
            {@html m.home_section3_image_transparent1()}
            <span class="font-mono bg-warning-content/20 rounded-md"
                >#deface</span
            >
            {m.home_section3_image_transparent2()}
        </p>
    </div>
    <span class="flex gap-2 items-end">
        <h2 class="text-6xl">2.</h2>
        <p class="text-xl">{m.home_section3_step2()}</p>
    </span>

    <span class="flex gap-2 items-end">
        <h2 class="text-6xl">3.</h2>
        <p class="text-xl">
            {@html m.home_section3_step3()}
        </p>
    </span>
    <span class="flex gap-2 items-end">
        <h2 class="text-6xl">4.</h2>
        <p class="text-xl">
            {@html m.home_section3_step4()}
        </p>
    </span>
</section>
<PixelsTransition to="var(--color-base-100)" from="var(--color-primary)" />
<section
    id="support"
    class="w-full py-20 flex flex-col gap-4 md:items-center px-4"
>
    <code class="text-primary scale-y-[.85]">{m.home_section4_subtitle()}</code>
    <h1 class="text-2xl md:text-4xl text-balance">
        {m.home_section4_title()}
    </h1>
    <div class="flex flex-col md:flex-row gap-4">
        <a
            target="_blank"
            href="https://discord.gg/WXWqhZ3udH"
            class="flex relative md:flex-col gap-4 p-4 rounded-box md:items-center overflow-hidden border border-base-300 hover:scale-[.98] transition-all before:absolute before:inset-0 before:transition-colors hover:before:bg-base-content/10"
        >
            <svg
                viewBox="0 0 256 199"
                class="size-10"
                xmlns="http://www.w3.org/2000/svg"
                preserveAspectRatio="xMidYMid"
                ><path
                    d="M216.856 16.597A208.502 208.502 0 0 0 164.042 0c-2.275 4.113-4.933 9.645-6.766 14.046-19.692-2.961-39.203-2.961-58.533 0-1.832-4.4-4.55-9.933-6.846-14.046a207.809 207.809 0 0 0-52.855 16.638C5.618 67.147-3.443 116.4 1.087 164.956c22.169 16.555 43.653 26.612 64.775 33.193A161.094 161.094 0 0 0 79.735 175.3a136.413 136.413 0 0 1-21.846-10.632 108.636 108.636 0 0 0 5.356-4.237c42.122 19.702 87.89 19.702 129.51 0a131.66 131.66 0 0 0 5.355 4.237 136.07 136.07 0 0 1-21.886 10.653c4.006 8.02 8.638 15.67 13.873 22.848 21.142-6.58 42.646-16.637 64.815-33.213 5.316-56.288-9.08-105.09-38.056-148.36ZM85.474 135.095c-12.645 0-23.015-11.805-23.015-26.18s10.149-26.2 23.015-26.2c12.867 0 23.236 11.804 23.015 26.2.02 14.375-10.148 26.18-23.015 26.18Zm85.051 0c-12.645 0-23.014-11.805-23.014-26.18s10.148-26.2 23.014-26.2c12.867 0 23.236 11.804 23.015 26.2 0 14.375-10.148 26.18-23.015 26.18Z"
                    fill="#5865F2"
                /></svg
            >
            <div class="flex flex-col">
                <p class="font-mono text-xs opacity-50 text-primary self-start">
                    {m.home_section4_members()}
                </p>
                <h2 class="text-2xl font-semibold">The Tipping Hat</h2>
            </div>
        </a>
        <a
            target="_blank"
            href="https://github.com/SwingTheVine/Wplace-BlueMarble/discussions"
            class="flex relative md:flex-col gap-4 p-4 rounded-box md:items-center md:justify-between overflow-hidden border border-base-300 hover:scale-[.98] transition-all before:absolute before:inset-0 before:transition-colors hover:before:bg-base-content/10"
        >
            <svg
                class="size-10"
                viewBox="0 0 1024 1024"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                ><path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M8 0C3.58 0 0 3.58 0 8C0 11.54 2.29 14.53 5.47 15.59C5.87 15.66 6.02 15.42 6.02 15.21C6.02 15.02 6.01 14.39 6.01 13.72C4 14.09 3.48 13.23 3.32 12.78C3.23 12.55 2.84 11.84 2.5 11.65C2.22 11.5 1.82 11.13 2.49 11.12C3.12 11.11 3.57 11.7 3.72 11.94C4.44 13.15 5.59 12.81 6.05 12.6C6.12 12.08 6.33 11.73 6.56 11.53C4.78 11.33 2.92 10.64 2.92 7.58C2.92 6.71 3.23 5.99 3.74 5.43C3.66 5.23 3.38 4.41 3.82 3.31C3.82 3.31 4.49 3.1 6.02 4.13C6.66 3.95 7.34 3.86 8.02 3.86C8.7 3.86 9.38 3.95 10.02 4.13C11.55 3.09 12.22 3.31 12.22 3.31C12.66 4.41 12.38 5.23 12.3 5.43C12.81 5.99 13.12 6.7 13.12 7.58C13.12 10.65 11.25 11.33 9.47 11.53C9.76 11.78 10.01 12.26 10.01 13.01C10.01 14.08 10 14.94 10 15.21C10 15.42 10.15 15.67 10.55 15.59C13.71 14.53 16 11.53 16 8C16 3.58 12.42 0 8 0Z"
                    transform="scale(64)"
                    fill="#ffff"
                /></svg
            >
            <div class="flex flex-col">
                <h2 class="text-2xl font-semibold">
                    {m.home_section4_github()}
                </h2>
            </div>
        </a>
    </div>
</section>
<section
    id="security"
    class="w-full py-20 flex flex-col gap-4 md:items-center lg:px-40 md:px-20 px-4"
>
    <code class="text-primary scale-y-[.85]">{m.home_section5_subtitle()}</code>
    <h1 class="text-2xl md:text-4xl text-balance">
        {m.home_section5_title()}
    </h1>
    <p>
        {@html m.home_section5_description()}
    </p>
</section>
<section
    id="contributing"
    class="w-full py-20 flex flex-col gap-4 md:items-center lg:px-40 md:px-20 px-4"
>
    <code class="text-primary scale-y-[.85]">{m.home_section6_subtitle()}</code>
    <h1 class="text-2xl md:text-4xl text-balance md:text-center">
        {m.home_section6_title()}
    </h1>
    <p class="text-lg">
        {@html m.home_section6_description()}
    </p>
</section>
